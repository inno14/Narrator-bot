    ...
    if (wnum.length > 0) {
      ...
      args[0] = allkill[wnum.indexOf(highest)]
        ...
        for (let z = 0; z < hacker.length; z++) {
            let chan1 = message.guild.channels.cache.get(hacker[z])
            for (let y = 1; y <= alive.members.size + dead.members.size; y++) {
                ...
                let hack = db.get(`hack_${chan1.id}`) || []
                if (hack.length > 0) {
                    let guys = []
                ...
                for (let j = 0; j < chanE.length; j++) {
                    ...
                    if (tempchan.permissionsFor(guy).has(["VIEW_CHANNEL", "READ_MESSAGE_HISTORY"])) {
                        j = 99
                        chanE = message.guild.channels.cache.get(chanE[j])
                    }
                }
